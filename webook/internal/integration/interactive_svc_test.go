	...
	testCases := []struct {
	...
	}{
		{
			...
			after: func(t *testing.T) {
				...
				defer cancel()
				var data dao.Interactive
				err := s.db.Where("id = ?", 11).First(&data).Error
				assert.NoError(t, err)
				assert.True(t, data.Utime > 7)
				data.Utime = 0
				assert.Equal(t, dao.Interactive{
				...
				err = s.db.Where("biz = ? AND biz_id = ? AND uid = ?",
					"test", 12, 123).First(&likeBiz).Error
				assert.NoError(t, err)
				...
				cnt, err := s.rdb.HGet(ctx, "interactive:test:12", "like_cnt").Int()
				assert.NoError(t, err)
				assert.Equal(t, 4, cnt)
				err = s.rdb.Del(ctx, "interactive:test:2").Err()
				assert.NoError(t, err)
		...
		},
		{
			...
			after: func(t *testing.T) {
				...
				defer cancel()
				var data dao.Interactive
				err := s.db.Where("biz_id = ? AND biz = ?", 13, "test").First(&data).Error
				assert.NoError(t, err)
				assert.True(t, data.Utime > 0)
				assert.True(t, data.Ctime > 0)
				assert.True(t, data.Id > 0)
				...
				var likeBiz dao.UserLikeBiz
				err = s.db.Where("biz = ? AND biz_id = ? AND uid = ?",
					"test", 13, 123).First(&likeBiz).Error
				assert.NoError(t, err)
				assert.True(t, likeBiz.Id > 0)
				assert.True(t, likeBiz.Ctime > 0)
				assert.True(t, likeBiz.Utime > 0)
				...
				cnt, err := s.rdb.Exists(ctx, "interactive:test:13").Result()
				assert.NoError(t, err)
				assert.Equal(t, int64(0), cnt)
			},
			biz:   "test",
	...
	testCases := []struct {
	...
	}{
		{
			...
			before: func(t *testing.T) {
				...
				assert.NoError(t, err)
				err = s.rdb.HSet(ctx, "interactive:test:22",
					"like_cnt", 3).Err()
				assert.NoError(t, err)
			},
			after: func(t *testing.T) {
				...
				defer cancel()
				var data dao.Interactive
				err := s.db.Where("id = ?", 21).First(&data).Error
				assert.NoError(t, err)
				assert.True(t, data.Utime > 7)
				data.Utime = 0
				assert.Equal(t, dao.Interactive{
				...
				var likeBiz dao.UserLikeBiz
				err = s.db.Where("id = ?", 21).First(&likeBiz).Error
				assert.NoError(t, err)
				...
				cnt, err := s.rdb.HGet(ctx, "interactive:test:22", "like_cnt").Int()
				assert.NoError(t, err)
